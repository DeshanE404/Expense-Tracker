...
// helps in converting data to ISO time
...
const Income = () => {
  ...
  const [showModal, setShowModal] = useState(false); // show add transaction model
  ...
  const chartData = useMemo(() => {
    ...
    filteredTransactions.forEach((transaction) => {
      ...
      if (point) {
        point.income += Math.round(Number(transaction.amount || 0));
      }
    });
  ...
  useEffect(() => {
    ...
    refreshTransactions();
  }, [fetchOverview, timeFrame, refreshTransactions]);
  ...
  const transactionsCount = useMemo(
    () => overview.numberOfTransactions ? overview.numberOfTransactions : filteredTransactions.length,
...
//to download with excel
  const handleExport = useCallback(async () => {
    try {
      const res = await axios.get(`${API_BASE}/income/download`, {
        headers: getAuthHeaders(),
        responseType: "blob",
      });
    ...
    <div className={styles.wrapper}>
      ...
      <div className={styles.summaryGrid}>
        <FinancialCard
          ...
          label="Total Income"
          value={`Rs ${Number(totalIncome || 0).toLocaleString()}`}
          additionalContent={
        ...
        <FinancialCard
          ...
          value={`Rs ${Number(averageIncome || 0).toLocaleString()}`}
          additionalContent={
            <div className="mt-2 text-xs text-gray-500 flex items-center">
...
// it shows 3 boxes with details
// Total , Avarage , Transactions